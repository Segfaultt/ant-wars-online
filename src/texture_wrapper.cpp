...
texture_wrapper::~texture_wrapper() { free(); }
...
bool texture_wrapper::load_texture(std::string path, SDL_Renderer *renderer) {
      ...
      if (new_surface == NULL) {
            ...
            SDL_SetColorKey(new_surface, SDL_TRUE,
                            SDL_MapRGB(new_surface->format, 0xff, 0x00, 0xff));
...
int texture_wrapper::get_height() { return height; }

int texture_wrapper::get_width() { return width; }
...
void texture_wrapper::render(int x, int y, double angle,
      ...
      SDL_RenderCopyEx(renderer, texture, NULL, &render_quad, angle, NULL,
                       SDL_FLIP_NONE);
}
...
bool texture_wrapper::load_text(std::string text, SDL_Color colour,
                                std::string font_path, int size,
                                SDL_Renderer *renderer) {
      bool success = true;

      this->free();
      TTF_Font *font = TTF_OpenFont(font_path.c_str(), size);
      SDL_Surface *message_as_surface =
          TTF_RenderText_Blended(font, text.c_str(), colour);
      if (message_as_surface == NULL) {
            std::cerr << "Could not render text to surface: " << SDL_GetError()
                      << std::endl;
            success = false;
      } else {
            texture =
                SDL_CreateTextureFromSurface(renderer, message_as_surface);
            if (texture == NULL) {
                  std::cout
                      << "Unable to create texture of text: " << SDL_GetError()
                      << std::endl;
                  success = false;
            } else {
